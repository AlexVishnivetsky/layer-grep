...
def _apply_version_migration(
    ...
    mismatch; it must not commit itself - this function commits once afterward, matching
    every original migration block's own trailing conn.commit()."""
...
def _wipe_for_chunker_version(conn: sqlite3.Connection) -> None:
...
def open_db(db_path: Path, model_name: str, project_root: Path) -> sqlite3.Connection:
    ...
    _apply_version_migration(conn, "json_index_version", JSON_INDEX_VERSION, _wipe_for_json_index_version)
    ...
    _apply_version_migration(conn, "chunker_version", CHUNKER_VERSION, _wipe_for_chunker_version)
    return conn
